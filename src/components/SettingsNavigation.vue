...
function selectTab(id: SettingsTabId) {
  if (id === 'integrations') emit('selectIntegrations')
...
<template>
  <aside class="settings-section-rail" role="tablist" aria-label="Settings sections" @keydown="onTabKeydown">
      ...
      <template v-for="tab in group.items" :key="tab.id">
        <div v-if="tab.id === 'integrations'" class="settings-section-parent-row">
          <button
            ...
            @click="emit('selectIntegrations')"
          >{{ tab.label }}</button>
          <button
            type="button"
            class="settings-section-expand"
          ...
          ><span :class="{ open: integrationsExpanded }" aria-hidden="true">›</span></button>
        </div>
        <button
          v-else
          :id="`tab-${tab.id}`"
        ...
        <div v-if="tab.id === 'integrations'" :class="['settings-section-subgroup', { open: integrationsExpanded }]">
          <button
  ...
  <label class="settings-section-picker">
    ...
    <select :value="mobileValue" @change="onMobileChange">
      <optgroup v-for="group in groups" :key="group.name" :label="group.name">
        <option v-for="tab in group.items" :key="tab.id" :value="tab.id">{{ tab.label }}</option>
      </optgroup>
...
.settings-section-item:focus-visible, .settings-section-expand:focus-visible, .settings-section-subitem:focus-visible { outline: 2px solid var(--focus-ring); outline-offset: -2px; }
.settings-section-parent-row { display: grid; grid-template-columns: minmax(0, 1fr) 32px; }
.settings-section-expand { display: grid; place-items: center; padding: 0; color: var(--text-muted); background: transparent; border: 0; border-radius: 5px; font-size: 18px; }
.settings-section-expand:hover { color: var(--text-primary); background: var(--bg-hover); }
.settings-section-expand span { transition: transform 180ms ease; }
.settings-section-expand span.open { color: var(--accent); transform: rotate(90deg); }
...
@media (prefers-reduced-motion: reduce) {
  .settings-section-expand span, .settings-section-subgroup, .settings-section-subitem { transition-duration: 1ms; }
}
</style>
