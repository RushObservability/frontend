...
function requestHeaders(input: HeadersInit | undefined, tenant: string): Record<string, string> {
  const headers: Record<string, string> = {}
  if (input instanceof Headers) input.forEach((value, key) => { headers[key] = value })
  else if (Array.isArray(input)) {
    for (const [key, value] of input) headers[key] = value
  } else if (input) Object.assign(headers, input)
...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  const execute = async (signal?: AbortSignal): Promise<string> => {
    const res = await authenticatedFetch(`${API_BASE}${path}`, {
      ...
      requestKey: method === 'GET' ? `api:${userId}:${tenant}:${path}` : undefined,
    })
    if (!res.ok) {
  ...
  if (analytics) {
    const body = await analyticsRequestCoordinator.run({
      userId,
      tenant,
      method,
      route: path,
  ...
  const operation = execute(options?.signal ?? undefined)
...
export function useApi() {
  ...
  async function getTrace(traceId: string): Promise<TraceResponse> {
    ...
    error.value = null
    try {
      return await request<TraceResponse>(`/traces/${encodePathSegment(traceId)}`)
    } catch (e: any) {
  ...
  async function queryEvents(
    ...
    try {
      return await request(`/query`, {
        method: 'POST',
        headers: workload ? { 'X-Rush-Workload': workload } : undefined,
  ...
  async function queryExplore<T extends RushEvent | LogRecord>(
    ...
    error.value = null
    try {
      return await request('/explore/search', {
        method: 'POST',
        body: JSON.stringify(req),
        signal,
      })
    } catch (e: any) {
      // A superseded Explore search is expected and should not replace current
      // rows with an error banner.
      if (e?.name !== 'AbortError') error.value = e.message
      throw e
    } finally {
      loading.value = false
  ...
  async function getDashboard(id: string, signal?: AbortSignal): Promise<DashboardWithWidgets> {
    return await request(`/dashboards/${encodePathSegment(id)}`, { signal })
  }
  ...
  async function deleteDashboard(id: string): Promise<void> {
    await request(`/dashboards/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function exportDashboard(id: string): Promise<DashboardExport> {
    return await request(`/dashboards/${encodePathSegment(id)}/export`)
  }
  ...
  async function createFromTemplate(templateId: string, name: string): Promise<Dashboard> {
    return await request(`/dashboard-templates/${encodePathSegment(templateId)}/create`, {
      method: 'POST',
  ...
  async function createWidget(dashId: string, data: Partial<Widget>): Promise<Widget> {
    return await request(`/dashboards/${encodePathSegment(dashId)}/widgets`, {
      method: 'POST',
      body: JSON.stringify(data),
    })
  }
  ...
  async function updateWidget(dashId: string, wid: string, data: Partial<Widget>): Promise<Widget> {
    return await request(`/dashboards/${encodePathSegment(dashId)}/widgets/${encodePathSegment(wid)}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
  }
  ...
  async function deleteWidget(dashId: string, wid: string): Promise<void> {
    await request(`/dashboards/${encodePathSegment(dashId)}/widgets/${encodePathSegment(wid)}`, { method: 'DELETE' })
  }
  ...
  async function updateChannel(id: string, data: { name: string; config: Record<string, unknown>; enabled: boolean }): Promise<NotificationChannel> {
    return await request(`/channels/${encodePathSegment(id)}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
  }
  ...
  async function deleteChannel(id: string): Promise<void> {
    await request(`/channels/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function testChannel(id: string): Promise<{ ok: boolean; message: string }> {
    return await request(`/channels/${encodePathSegment(id)}/test`, {
      method: 'POST',
    })
  }

  async function notifyChannel(id: string, payload: Record<string, unknown>): Promise<void> {
    await request(`/channels/${encodePathSegment(id)}/notify`, {
      method: 'POST',
  ...
  async function deleteMaintenanceWindow(id: string): Promise<void> {
    await request(`/maintenance-windows/${encodePathSegment(id)}`, { method: 'DELETE' })
  }
  ...
  async function deleteFunnel(id: string): Promise<void> {
    await request(`/funnels/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function runFunnel(id: string, from: string, to: string): Promise<FunnelResult> {
    return await request(`/funnels/${encodePathSegment(id)}/run`, { method: 'POST', body: JSON.stringify({ from, to }) })
  }

  // ── SLO API ──

  async function listSlos(signal?: AbortSignal): Promise<{ slos: Slo[] }> {
    return await request('/slos', { signal })
  }
  ...
  async function getSlo(id: string, signal?: AbortSignal): Promise<{ slo: Slo; events: SloEvent[] }> {
    return await request(`/slos/${encodePathSegment(id)}`, { signal })
  }

  async function updateSlo(id: string, data: Record<string, unknown>): Promise<Slo> {
    return await request(`/slos/${encodePathSegment(id)}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
  }
  ...
  async function deleteSlo(id: string): Promise<void> {
    await request(`/slos/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function listSloEvents(id: string): Promise<{ events: SloEvent[] }> {
    return await request(`/slos/${encodePathSegment(id)}/events`)
  }

  // ── Service Graph API ──

  async function serviceGraph(minutes?: number, signal?: AbortSignal): Promise<ServiceGraph> {
    const qs = minutes ? `?minutes=${minutes}` : ''
    return await request(`/services/graph${qs}`, { signal })
  }
  ...
  async function deleteApiKey(id: string): Promise<void> {
    await request(`/api-keys/${encodePathSegment(id)}`, { method: 'DELETE' })
  }
  ...
  async function deleteServiceLink(serviceName: string): Promise<void> {
    await request(`/service-links/${encodePathSegment(serviceName)}`, { method: 'DELETE' })
  }
  ...
  async function getLogContext(req: {
    time_range: { from: string; to: string }
    filters: Filter[]
    search?: string
    anchor: {
    ...
    before?: number
    after?: number
  }): Promise<{ rows: LogRecord[] }> {
    return await request('/logs/context', {
      method: 'POST',
      body: JSON.stringify(req),
    })
  }
  ...
  async function groupLogs(req: {
    ...
    limit?: number
    search?: string
  }, workload?: 'dashboard', signal?: AbortSignal): Promise<GroupResponse> {
  ...
  async function countLogs(req: {
    ...
    interval?: string
    search?: string
  }, workload?: 'dashboard', signal?: AbortSignal): Promise<CountBucket[]> {
    loading.value = true
    error.value = null
    try {
  ...
  async function getSystemMetrics(signal?: AbortSignal): Promise<string> {
    const { activeTenant } = useTenant()
  ...
  async function getStats(req: {
    time_range?: { from: string; to: string }
  }, signal?: AbortSignal): Promise<StatsResponse> {
  ...
  async function getIngestBuffer(signal?: AbortSignal): Promise<{ backend: string; pending_bytes: number; pending_count: number; max_bytes: number; used_pct: number; oldest_age_secs: number; committed_total: number }> {
    return await request('/ingest/buffer', { signal })
  }
  ...
  async function listAnomalyRules(signal?: AbortSignal): Promise<{ rules: AnomalyRule[] }> {
    return await request('/anomaly-rules', { signal })
  }
  ...
  async function getAnomalyRule(id: string): Promise<{ rule: AnomalyRule; events: AnomalyEvent[] }> {
    return await request(`/anomaly-rules/${encodePathSegment(id)}`)
  }
  ...
  async function deleteAnomalyRule(id: string): Promise<void> {
    await request(`/anomaly-rules/${encodePathSegment(id)}`, { method: 'DELETE' })
  }
  ...
  async function getAnomalyEvent(eventId: string): Promise<AnomalyEvent> {
    return await request(`/anomaly-events/${encodePathSegment(eventId)}`)
  }

  async function getEventCorrelations(eventId: string): Promise<CorrelationResponse> {
    return await request(`/anomaly-events/${encodePathSegment(eventId)}/correlations`)
  }

  async function analyzeAnomalyEvent(eventId: string, additionalContext?: string): Promise<AIAnalysisResponse> {
    return await request(`/anomaly-events/${encodePathSegment(eventId)}/analyze`, {
      method: 'POST',
  ...
  async function rumSession(id: string): Promise<{ events: RumRecord[] }> {
    return await request(`/rum/session/${encodePathSegment(id)}`)
  }
  ...
  async function getArgoApp(name: string): Promise<ArgoAppDetail> {
    return await request<ArgoAppDetail>(`/argocd/applications/${encodePathSegment(name)}`)
  }
  ...
  async function getFluxResource(kind: string, name: string): Promise<FluxResourceDetail> {
    return await request<FluxResourceDetail>(
      `/fluxcd/resources/${encodePathSegment(kind)}/${encodePathSegment(name)}`,
    )
  }
  ...
  async function getK8sResources(kind: string, namespace?: string): Promise<K8sResource[]> {
    ...
    const res = await request<{ resources: K8sResource[] }>(`/kubernetes/resources/${encodePathSegment(kind)}${qs}`)
    return res.resources || []
  }
  ...
  async function getK8sResource(kind: string, namespace: string, name: string): Promise<K8sResourceDetail> {
    ...
    return await request<K8sResourceDetail>(
      `/kubernetes/resources/${encodePathSegment(kind)}/${encodePathSegment(ns)}/${encodePathSegment(name)}`,
    )
  }
  ...
  async function getCustomSkill(id: string): Promise<CustomSkill> {
    return await request<CustomSkill>(`/custom-skills/${encodePathSegment(id)}`)
  }
  ...
  async function updateGroup(id: string, data: {
    ...
    return await request(`/groups/${encodePathSegment(id)}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
  }
  ...
  async function deleteGroup(id: string): Promise<void> {
    await request(`/groups/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function setGroupTenants(id: string, tenantIds: string[]): Promise<Group> {
    return await request(`/groups/${encodePathSegment(id)}/tenants`, {
      method: 'PUT',
  ...
  async function getUserGroups(userId: string): Promise<{ group_ids: string[] }> {
    return await request(`/users/${encodePathSegment(userId)}/groups`)
  }
  ...
  async function deleteTenant(id: string): Promise<void> {
    await request(`/tenants/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function toggleTenant(id: string, enabled: boolean): Promise<Tenant> {
    return await request<Tenant>(`/tenants/${encodePathSegment(id)}/toggle`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function setTenantAuthRequired(id: string, auth_required: boolean): Promise<Tenant> {
    return await request<Tenant>(`/tenants/${encodePathSegment(id)}/auth`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function setTenantIngestAuthRequired(id: string, ingest_auth_required: boolean): Promise<Tenant> {
    return await request<Tenant>(`/tenants/${encodePathSegment(id)}/ingest-auth`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function getTenantRetention(tenantId: string): Promise<TenantRetention> {
    return await request<TenantRetention>(`/tenants/${encodePathSegment(tenantId)}/retention`)
  }
  ...
  async function deleteTenantRetention(tenantId: string, signal: string): Promise<void> {
    await request(`/tenants/${encodePathSegment(tenantId)}/retention/${encodePathSegment(signal)}`, { method: 'DELETE' })
  }

  async function getTenantSignals(tenantId: string): Promise<TenantSignals> {
    return await request<TenantSignals>(`/tenants/${encodePathSegment(tenantId)}/signals`)
  }
  ...
  async function deleteUser(id: string): Promise<void> {
    await request(`/users/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function changeUserPassword(id: string, password: string): Promise<void> {
    await request(`/users/${encodePathSegment(id)}/password`, {
      method: 'PUT',
  ...
  async function toggleUser(id: string, enabled: boolean): Promise<User> {
    return await request<User>(`/users/${encodePathSegment(id)}/toggle`, {
      method: 'PUT',
      body: JSON.stringify({ enabled }),
    })
  }
  ...
  async function login(username: string, password: string): Promise<AuthResponse> {
    ...
    // Keep the frontend contract cookie-only even if an older API build sends
    // a legacy token property. Session bearers must never leave HttpOnly cookies.
  ...
  async function listAuthSessions(admin = false): Promise<{ sessions: AuthSession[] }> {
    return await request(admin ? '/auth/admin/sessions' : '/auth/sessions')
  }

  async function revokeAuthSession(sessionId: string, admin = false): Promise<void> {
    const prefix = admin ? '/auth/admin/sessions' : '/auth/sessions'
    await request(`${prefix}/${encodePathSegment(sessionId)}`, { method: 'DELETE' })
  }
  ...
  async function deleteSsoProvider(id: string): Promise<void> {
    await request(`/sso/providers/${encodePathSegment(id)}`, { method: 'DELETE' })
  }
  ...
  async function updateIdpGroupMapping(id: string, data: {
    ...
    return await request(`/sso/mappings/${encodePathSegment(id)}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
  }
  ...
  async function deleteIdpGroupMapping(id: string): Promise<void> {
    await request(`/sso/mappings/${encodePathSegment(id)}`, { method: 'DELETE' })
  }
  ...
  function promHeaders(workload?: 'dashboard'): Record<string, string> {
    const { activeTenant } = useTenant()
    return {
      'X-Rush-Tenant': activeTenant.value,
      ...(workload ? { 'X-Rush-Workload': workload } : {}),
    }
  }

  async function promQuery(query: string, time?: number, signal?: AbortSignal): Promise<PromVectorResponse> {
    const { activeTenant } = useTenant()
    const params = new URLSearchParams({ query })
    if (time !== undefined) params.set('time', String(time))
    const res = await authenticatedFetch(`/prom/api/v1/query?${params}`, { headers: promHeaders(), signal }, { requestKey: `prom:${activeTenant.value}:query:${params}` })
    if (!res.ok) {
      try { await res.body?.cancel() } catch { /* best effort: release the body */ }
      throw responseError(res, 'Prometheus query')
    }
  ...
  async function promQueryRange(
    query: string, start: number, end: number, step: number, workload?: 'dashboard', signal?: AbortSignal
    ...
    const res = await authenticatedFetch(`/prom/api/v1/query_range?${params}`, { headers: promHeaders(workload), signal }, { requestKey: `prom:${activeTenant.value}:query-range:${params}:${workload || 'interactive'}` })
    if (!res.ok) {
      try { await res.body?.cancel() } catch { /* best effort: release the body */ }
      throw responseError(res, 'Prometheus range query')
    }
    const json = await res.json()
  ...
  async function promLabels(match?: string): Promise<string[]> {
    ...
    if (!res.ok) {
      try { await res.body?.cancel() } catch { /* best effort: release the body */ }
      throw responseError(res, 'Prometheus label query')
    }
    const json = await res.json()
  ...
  async function promLabelValues(label: string, match?: string): Promise<string[]> {
    const { activeTenant } = useTenant()
    const qs = match ? `?match[]=${encodeURIComponent(match)}` : ''
    const res = await authenticatedFetch(`/prom/api/v1/label/${encodePathSegment(label)}/values${qs}`, { headers: promHeaders() }, { requestKey: `prom:${activeTenant.value}:label-values:${label}:${qs}` })
    if (!res.ok) {
      try { await res.body?.cancel() } catch { /* best effort: release the body */ }
      throw responseError(res, 'Prometheus label-values query')
    }
    const json = await res.json()
    return json.data as string[]
  }
  ...
  async function getMonitor(id: string): Promise<{ monitor: Monitor; events: MonitorEvent[] }> {
    return await request(`/monitors/${encodePathSegment(id)}`)
  }
  ...
  async function deleteMonitor(id: string): Promise<void> {
    await request(`/monitors/${encodePathSegment(id)}`, { method: 'DELETE' })
  }

  async function listMonitorEvents(id: string): Promise<{ events: MonitorEvent[] }> {
    return await request(`/monitors/${encodePathSegment(id)}/events`)
  }
  ...
  async function muteMonitor(id: string): Promise<void> {
    await request(`/monitors/${encodePathSegment(id)}/mute`, { method: 'POST' })
  }

  async function unmuteMonitor(id: string): Promise<void> {
    await request(`/monitors/${encodePathSegment(id)}/unmute`, { method: 'POST' })
  }
  ...
  async function getQueryLimits(): Promise<QueryLimitsResponse> {
    return await request('/settings/query-limits')
  }

  async function setQueryLimits(limits: QueryLimitsConfig): Promise<QueryLimitsResponse> {
    return await request('/settings/query-limits', {
      method: 'PUT',
  ...
  async function updateMetricFirewall(id: string, input: MetricFirewallInput): Promise<MetricFirewallRule> {
    return await request(`/metric-firewall/${encodePathSegment(id)}`, {
      method: 'PUT',
      body: JSON.stringify(input),
    })
  }
  ...
  async function deleteMetricFirewall(id: string): Promise<void> {
    await request(`/metric-firewall/${encodePathSegment(id)}`, { method: 'DELETE' })
  }
  ...
  async function exportExplore(opts: {
    ...
    let downloadResponse = res
    if (res.status === 202) {
      type ExportJob = {
        state: 'queued' | 'running' | 'completed' | 'failed' | 'cancelled' | 'expired'
        status_url: string
        download_url?: string
        error?: string
      }
      const safeJobPath = (value: string): string => {
        if (!/^\/api\/v1\/exports\/[0-9a-f-]{36}(?:\/download)?$/.test(value)) {
          throw new Error('Export returned an invalid job location.')
        }
        return value
      }
      let job = await res.json() as ExportJob
      const deadline = Date.now() + 10 * 60 * 1000
      while (job.state === 'queued' || job.state === 'running') {
        if (Date.now() >= deadline) throw new Error('Export is still running. Try again shortly.')
        await new Promise(resolve => window.setTimeout(resolve, 1000))
        const statusResponse = await authenticatedFetch(safeJobPath(job.status_url), {
          credentials: 'same-origin',
          headers: { 'X-Rush-Tenant': activeTenant.value },
        })
        if (!statusResponse.ok) {
          try { await statusResponse.body?.cancel() } catch { /* best effort */ }
          throw new Error(safeApiErrorMessage(statusResponse.status, 'Export status'))
        }
        job = await statusResponse.json() as ExportJob
      }
      if (job.state !== 'completed' || !job.download_url) {
        throw new Error(job.error || `Export ${job.state}.`)
      }
      downloadResponse = await authenticatedFetch(safeJobPath(job.download_url), {
        credentials: 'same-origin',
        headers: { 'X-Rush-Tenant': activeTenant.value },
      })
      if (!downloadResponse.ok) {
        try { await downloadResponse.body?.cancel() } catch { /* best effort */ }
        throw new Error(safeApiErrorMessage(downloadResponse.status, 'Export download'))
      }
    }
