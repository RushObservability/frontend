...
export function useApi() {
  ...
  async function getDashboard(id: string, signal?: AbortSignal): Promise<DashboardWithWidgets> {
    return await request(`/dashboards/${id}`, { signal })
  }
  ...
  async function getSlo(id: string, signal?: AbortSignal): Promise<{ slo: Slo; events: SloEvent[] }> {
    return await request(`/slos/${id}`, { signal })
  }
  ...
  async function revokeAuthSession(sessionId: string, admin = false): Promise<void> {
    ...
    await request(`${prefix}/${encodeURIComponent(sessionId)}`, { method: 'DELETE' })
  }
  ...
  return {
    ...
    getTrace, queryEvents, queryExplore, queryCount, queryTimeseries, openInvestigationStream, getServices, serviceGraph, serviceTimeBreakdown, serviceTimeBreakdownTimeseries, serviceLatencyHistogram, serviceEndpoints, serviceErrors, getLicense, submitExplain, getExplainJob, suggestValues, queryGroup,
    ...
    getFeatures, getRuntimeConfig, setExportMaxRows, getQueryLimits, setQueryLimits, getSreAgentSettings, setSreAgentSettings, getSreAgentModels, getSreAgentOptions, setSreAgentEnabled, setSreAgentTenantAccess, getDeployMarkersSetting, setDeployMarkersEnabled, getRumSetting, setRumEnabled, getCloudwatchSetting, setCloudwatchSetting, exportExplore,
