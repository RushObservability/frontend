...
export type QueryWorkloadClass = 'interactive' | 'dashboard' | 'background' | 'export' | 'integration'

export interface QueryWorkloadBudget {
  global_concurrency: number
  per_tenant_concurrency: number
  queue_timeout_ms: number
  request_timeout_secs: number
  max_time_range_secs: number
...
export type QueryLimitsConfig = Record<QueryWorkloadClass, QueryWorkloadBudget>

export interface QueryLimitsResponse {
  limits: QueryLimitsConfig
  defaults?: QueryLimitsConfig
}
...
export interface QueryRequest {
  ...
  search?: string
  /**
   * Authenticated keyset cursor from a prior response. The backend binds it to
   * the active tenant, filters, search text, time range, and projection.
   */
  cursor?: string
  /**
...
export interface ExploreSearchRequest {
  ...
  interval?: string
  group_by?: string
  /** Return result rows and pagination metadata. Defaults to true. */
  include_rows?: boolean
  /** Return the exact total, histogram, facets, and groups. Defaults to true. */
  include_summary?: boolean
}

export interface ExploreSearchResponse<T extends RushEvent | LogRecord = RushEvent | LogRecord> {
  signal: 'spans' | 'logs'
  rows: T[]
  next_cursor?: string
  count: {
    value: number
    kind: ExploreCountKind
  }
  summary: {
    histogram: CountBucket[]
    facets: {
      services: ExploreSummaryValue[]
      statuses: ExploreSummaryValue[]
      methods: ExploreSummaryValue[]
    }
    groups: ExploreSummaryValue[]
  ...
  query_stats: {
    ...
    matched_rows: number
    matched_logical_bytes: number
    time_to_first_results_ms: number
...
// Kubernetes access recording. The API returns these fields only to admins.

export interface KubernetesClientSession {
  session_id: string
  username: string
  cluster_id: string
  hostname: string
...
export interface KubernetesGatewayCluster {
  gateway_id: string
  cluster_id: string
  configured: boolean
...
export type KubernetesRbacRoleKind = 'view' | 'edit' | 'admin' | 'existing' | 'custom'
export type KubernetesRbacScope = 'cluster' | 'namespaces'
export type KubernetesRbacClusterMatch = 'single' | 'all' | 'pattern'

export interface KubernetesRbacGrantInput {
  group_id: string
  cluster_id: string
  cluster_match: KubernetesRbacClusterMatch
  cluster_pattern: string
  name: string
  role_kind: KubernetesRbacRoleKind
  role_name: string
  scope: KubernetesRbacScope
  namespaces: string[]
  rules: KubernetesRbacRule[]
}

export interface KubernetesRbacGrant extends KubernetesRbacGrantInput {
  id: string
  tenant_id: string
...
export interface KubernetesLoggingSettings {
  max_session_seconds: number
  min_session_seconds: number
  max_allowed_session_seconds: number
  active_clients: KubernetesClientSession[]
  rbac_grants: KubernetesRbacGrant[]
  gateways: KubernetesGatewayCluster[]
  available_clusters: string[]
}

export type KubernetesAccessSourceKind =
  | 'gateway'
  | 'kubernetes_audit_webhook'
  | 'rush_cli'
  | string

export type KubernetesRecordingState =
  | 'complete'
  | 'partial'
  | 'failed'
  | 'not_recorded'
...
export interface KubernetesAccessQueryParams {
  from?: string
  to?: string
  actor?: string
  cluster?: string
...
export interface KubernetesAccessResultSummary {
  preview?: unknown
  content?: unknown
  artifact_id?: string
  truncated?: boolean
...
export interface KubernetesAccessNetwork {
  ...
  client_reported_private_ips?: string[]
  [key: string]: unknown
}

export interface KubernetesAccessClientReport {
  argv?: string[]
  cli_version?: string
  os?: string
  arch?: string
...
export interface KubernetesSessionArtifact {
  id?: string
  session_id?: string
  pod?: string
  container?: string
...
export type KubernetesSessionStream =
  | 'stdin'
  | 'stdout'
  | 'stderr'
  | 'error'
...
export interface KubernetesSessionChunk {
  id: string
  session_id: string
  event_id: string
  gateway_id: string
  ...
  recording_state: KubernetesRecordingState
  offset_ms: number
  data: string
  byte_count: number
  redaction_count: number
...
export interface KubernetesSessionChunkListResponse {
  chunks: KubernetesSessionChunk[]
  has_more: boolean
  next_sequence?: number
}

export interface KubernetesAccessEvent {
  id: string
  cluster_id?: string
  cluster_name?: string
  gateway_id?: string
  ...
  actor_name?: string
  actor_type?: 'user' | 'api_key' | 'system' | string
  actor_username?: string
  kube_username?: string
  kube_groups?: string[]
  source_kind: KubernetesAccessSourceKind
  client_reported?: KubernetesAccessClientReport | null
  observed_network?: KubernetesAccessNetwork | null
  verb: string
  http_method?: string
  api_group?: string
  api_version?: string
...
export interface KubernetesAccessListResponse {
  events: KubernetesAccessEvent[]
  limit?: number
  offset?: number
  total?: number
  next_cursor?: string
...
export interface KubernetesAccessDetailResponse {
