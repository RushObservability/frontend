...
async function loadGraphData(rethrow: boolean, signal?: AbortSignal) {
  if (graphRequest) return graphRequest
  const run = (async () => {
  graphLoading.value = true
  try {
    const result = await api.serviceGraph(graphMinutes.value, signal)
    graphNodes.value = result.nodes
    graphEdges.value = result.edges
  } catch (error) {
...
const graphLoop = usePollingTask({ category: 'service_graph', intervalMs: 30_000, run: ({ signal }) => loadGraphData(true, signal) })
...
<template>
  <div class="services-view">
    ...
    <EmptyState
      v-if="!apmEnabled"
      class="card"
      title="APM is disabled for this tenant"
      description="Enable the APM signal in Settings → Tenants to use the service catalog, dependency graph, and funnels."
      icon="!"
    />

    <!-- Catalog View -->
    <template v-if="apmEnabled && viewMode === 'catalog'">
      <EmptyState v-if="!namesLoaded" class="card" title="Loading services" description="Reading services from recent trace data." icon="◌" tone="loading" />

      <EmptyState
        v-else-if="catalogRows.length === 0 && !catalogLoading"
        class="card"
        title="No services in this range"
        description="Try a wider time range or check that your services are sending traces."
      />
    ...
    <template v-if="apmEnabled && viewMode === 'graph'">
      <EmptyState v-if="graphLoading && graphNodes.length === 0" class="card" title="Loading service map" description="Building dependencies from recent traces." icon="◌" tone="loading" />

      <EmptyState
        v-else-if="graphNodes.length === 0"
        class="card"
        title="No service map in this range"
        description="Try a wider time range or check that your services are sending traces."
      />
