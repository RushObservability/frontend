...
function streamEventAt(index: number): StreamEvent {
  return logStream.value[index]!
}

function streamEventKeyAt(index: number): string {
  const event = logStream.value[index]
  return event ? `${event.span_id}:${event.timestamp}:${event.name}:${index}` : `event:${index}`
}
    ...
    <template v-else-if="trace">
      ...
      <div v-if="logStream.length" class="log-stream card fade-in" style="animation-delay: 200ms">
        ...
        <VirtualTable
          ...
          :count="logStream.length"
          :item-key="streamEventKeyAt"
          variable
          aria-label="Trace log stream"
        >
        <template #default="{ index: i }">
        <template v-for="ev in [streamEventAt(i)]" :key="streamEventKeyAt(i)">
          <div
            class="log-row"
