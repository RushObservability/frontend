...
async function loadConfigs(rethrow = false, signal?: AbortSignal) {
  try {
    const resp = await api.listAnomalyRules(signal)
    savedConfigs.value = resp.rules.map(fromApiRule)
  } catch (error) {
    if (signal?.aborted) return
    if (!configsLoaded.value) savedConfigs.value = []
    if (rethrow) throw error
  } finally {
    if (!signal?.aborted) configsLoaded.value = true
  }
}
...
// ═══ Data fetching ═══
async function fetchMetrics(names: string[], labels?: string[], rangeIdx?: number, signal?: AbortSignal): Promise<MetricMonitor[]> {
  ...
  const results = await Promise.allSettled(
    names.map(async (name): Promise<MetricMonitor[]> => {
      ...
      const resp = await api.promQueryRange(query, start, end, step, undefined, signal)
...
async function fetchApmConfigData(config: AnomalyConfig, rangeIdx?: number, signal?: AbortSignal) {
...
async function fetchConfigData(config: AnomalyConfig, rangeIdx?: number, signal?: AbortSignal) {
  if (config.source === 'apm') {
    await fetchApmConfigData(config, rangeIdx, signal)
    return
  }
  if (!config.pattern) {
    configData.value[config.id] = []
  ...
  try {
    ...
    configData.value[config.id] = await fetchMetrics(names, config.splitLabels?.length ? config.splitLabels : undefined, rangeIdx, signal)
  } catch (error) {
    if (signal?.aborted) return
    configData.value[config.id] = []
    if (signal) throw error
...
async function refreshAll({ signal }: { signal: AbortSignal }) {
  ...
  try {
    ...
    if (visible.length) {
      const results = await Promise.allSettled(visible.map(c => fetchConfigData(c, activeRangeIdx(), signal)))
      const failure = results.find((result): result is PromiseRejectedResult => result.status === 'rejected')
      if (failure) throw failure.reason
    }
...
const liveLoop = usePollingTask({ category: 'anomaly', intervalMs: LIVE_INTERVAL, run: refreshAll })
