...
async function livePoll({ signal }: PollingRunContext) {
  ...
  try {
    ...
    const summaryPromise = settleExploreRequest({
      ...request,
      include_rows: false,
...
function applyExploreSummaryResponse(response: ExploreSearchResponse, isLogs: boolean) {
  ...
  exploreSummaryWarning.value = summaryUnavailable
  ...
  <div class="explore">
    ...
    <div class="search-section">
      <ExploreSearchToolbar
        :apm-enabled="apmEnabled"
        :view-mode="viewMode"
        :live="liveMode"
        :share-copied="shareCopied"
        @set-mode="mode => { setViewMode(mode); if (mode === 'logs') tracesOnly = false }"
      ...
      <div class="search-bar" :class="{ 'nl-mode': nlMode }">
        ...
        <input
          ...
          v-model="searchInput"
          class="search-input"
          aria-label="Search traces and logs"
    ...
    <div class="explore-body" :class="{ 'compare-active': compareModeActive && viewMode === 'spans' }">
      ...
      <div class="explore-main">
        ...
        <ExploreResultsState
          ...
          :mode="viewMode"
          :loading="searching"
          :error="viewMode !== 'logs' ? api.error.value : null"
          :has-results="viewMode === 'logs' ? logEntries.length > 0 : results.length > 0"
        />
