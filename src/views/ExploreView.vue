...
import type { RushEvent, Filter, CountBucket, QueryFilter, GroupResult, SavedQuery, TimeseriesBucket, TraceResponse, SpanNode, LogRecord, BubbleUpResponse, ExploreCountKind, ExploreSearchResponse } from '../types'
...
const formattedTotal = computed(() => {
  const value = total.value.toLocaleString()
  if (totalKind.value === 'capped') return `${value}+`
  if (totalKind.value === 'estimated') return `~${value}`
  return value
})
...
const paginationHasMore = ref(false)
const hasMore = computed(() => paginationHasMore.value)
...
const liveFollowing = ref(true)
const spanVirtualTable = ref<InstanceType<typeof VirtualTable> | null>(null)
const logVirtualTable = ref<InstanceType<typeof VirtualTable> | null>(null)
...
async function livePoll({ signal }: PollingRunContext) {
  ...
  try {
    ...
    const response = await api.queryExplore({
      ...
      search: searchParam,
      group_by: groupByField.value || undefined,
    }, signal)
    if (signal.aborted) return

    if (!isLogs) {
      const spanRows = response.rows as RushEvent[]
      const freshIds = new Set(spanRows.map(row => row.span_id))
    ...
    applyExploreResponse(response, isLogs)
  } catch (error) {
...
const liveLoop = usePollingTask({ category: 'explore_live', intervalMs: 10_000, run: livePoll })
...
function resumeLiveFollow() {
  liveFollowing.value = true
  const table = viewMode.value === 'logs' ? logVirtualTable.value : spanVirtualTable.value
  void nextTick(() => table?.resumeFollow())
}
...
function logEntryKeyAt(index: number): string {
  return logEntries.value[index]?.key || `log:${index}`
}
...
const serviceFacets = computed(() => {
  // Coordinated searches return counts across the full match set, not just the
  // loaded page. Fall back to visible rows when the summary timed out.
...
const statusFacets = computed(() => {
  ...
  if (ss.ok > 0 || ss.clientErr > 0 || ss.serverErr > 0) {
    return [
      { name: '2xx', label: viewMode.value === 'logs' ? 'Info' : 'OK', count: ss.ok, color: 'var(--ok)' },
      { name: '4xx', label: viewMode.value === 'logs' ? 'Warning' : 'Client Error', count: ss.clientErr, color: 'var(--warning)' },
      { name: '5xx', label: viewMode.value === 'logs' ? 'Error' : 'Server Error', count: ss.serverErr, color: 'var(--error)' },
    ]
  }
  if (viewMode.value === 'logs') {
...
function applyExploreResponse(response: ExploreSearchResponse, isLogs: boolean) {
  total.value = response.count.value
  totalKind.value = response.count.kind
  histogram.value = response.summary.histogram || []
  histogramPhase.value = response.errors.histogram ? 'idle' : 'done'
  exploreSummaryWarning.value = Object.keys(response.errors || {}).length
    ? 'Some summaries are temporarily unavailable. Results are still complete.'
    : null

  serverServiceCounts.value = new Map(
    (response.summary.facets.services || []).map(item => [item.key, item.count]),
  )
  serverMethodCounts.value = new Map(
    (response.summary.facets.methods || []).map(item => [item.key, item.count]),
  )
  let ok = 0
  let clientErr = 0
  let serverErr = 0
  for (const item of response.summary.facets.statuses || []) {
    if (isLogs) {
      if (item.key === 'error') serverErr += item.count
      else if (item.key === 'warn') clientErr += item.count
      else ok += item.count
    } else {
      const code = Number(item.key)
      if (code >= 500) serverErr += item.count
      else if (code >= 400) clientErr += item.count
      else ok += item.count
    }
  }
  serverStatusCounts.value = { ok, clientErr, serverErr }
  groupResults.value = (response.summary.groups || []).map(item => ({
    key: item.key,
    count: item.count,
  }))
  nextCursor.value = response.next_cursor || null
  paginationHasMore.value = Boolean(response.next_cursor)

  if (isLogs) {
    results.value = []
    otelLogs.value = response.rows as LogRecord[]
    ...
    results.value = response.rows as RushEvent[]
    matchHisto.value = []
    matchHistoLoading.value = false
  }
}
...
async function search(opts?: { skipHistory?: boolean }) {
  ...
  try {
    ...
    // The coordinator executes one row query and one single-scan summary query.
    // A superseding search aborts this request, preventing stale rows and allowing
    // the server/ClickHouse HTTP work to be dropped with the connection.
    histogram.value = []
...
async function loadMore() {
  ...
  try {
    ...
    if (viewMode.value === 'logs') {
      ...
      const page = await api.queryEvents({
        time_range: timeRange.value,
        filters: activeFilters,
        limit: PAGE_SIZE,
        cursor: nextCursor.value || undefined,
      ...
      results.value.push(...page.rows)
      nextCursor.value = page.next_cursor || null
      paginationHasMore.value = Boolean(page.next_cursor)
      total.value = page.total
      totalKind.value = page.total >= 10_000 ? 'capped' : 'exact'
    }
  } catch {
  ...
  scrollObserver = new IntersectionObserver(
    (entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting && hasMore.value && !loadingMore.value && !searching.value && !api.loading.value) {
          loadMore()
          break
        }
      }
...
async function openContextStream(entry: LogEntry) {
  ...
  // The backend walks both directions from the selected identity in one query.
  // This stays centered even in very dense streams and replaces the previous
  // sequence of progressively smaller time-window retries.
  const sourceRecord = entry._sourceLogIndex >= 0 ? otelLogs.value[entry._sourceLogIndex] : undefined
  let rows: LogRecord[] = []
  try {
    const response = await api.getLogContext({
      time_range: timeRange.value,
      filters,
      anchor: {
        timestamp_ns: sourceRecord?.TimestampNs || String(Math.trunc(entry.timestamp)),
    ...
    rows = response.rows
    contextStreamRadiusLabel.value = '250 before · 250 after'
  } catch {
    rows = []
  }
      ...
      <div class="explore-main">
        ...
        <div v-else-if="viewMode === 'spans' && apmResultMode === 'individual'" class="event-table card">
          ...
          <VirtualTable
            ...
            follow-edge="start"
            variable
            aria-label="Span results"
            @near-end="loadMore"
            @select="selectedRowIndex = $event"
            @activate="toggleRow($event)"
