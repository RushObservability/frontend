...
async function search() {
  ...
  try {
    ...
    if (logsResult.status === 'fulfilled') {
      ...
      nextCursor.value = logsResult.value.next_cursor || null
    }
    if (countResult.status === 'fulfilled') {
      histogram.value = countResult.value
      total.value = countResult.value.reduce((sum, bucket) => sum + bucket.count, 0)
    }
...
async function toggleRow(i: number) {
  ...
  if (expandedRow.value !== i || !row || (row.ResourceAttributes !== undefined && row.LogAttributes !== undefined)) return
  try {
    const detail = await api.getLogDetail(row)
    results.value[i] = { ...row, ...detail }
  } catch {
    // Keep the slim row visible if retention removed the wide detail concurrently.
  }
}
...
function logKeyAt(index: number): string {
  const row = results.value[index]
  return row
    ? `${row.TimestampNs || row.Timestamp}:${row.ServiceName}:${row.TraceId}:${row.SpanId}:${row.CursorHash || row.BodyHash || index}`
    : `log:${index}`
}

function logAt(index: number): LogRecord {
...
<template>
  <div class="logs-view">
    ...
    <!-- ═══ States ═══ -->
    <EmptyState
      v-if="api.loading.value && !results.length"
      class="card"
      title="Searching logs"
      description="Rush is querying the selected time range."
      icon="◌"
      tone="loading"
    />
    <EmptyState
      v-else-if="api.error.value"
      class="card"
      title="Log search failed"
      :description="api.error.value"
      icon="!"
      tone="error"
    />
    <EmptyState
      v-else-if="!results.length && !api.loading.value"
      class="card"
      title="No logs match this search"
      description="Try a wider time range, remove a filter, or search for a different term."
    />
    ...
    <div v-else class="event-table card">
      ...
      <template v-for="row in [logAt(i)]" :key="logKeyAt(i)">
        <div
          class="et-row"
          :class="{
