...
interface IdxRow extends Record<string, unknown> { schema: string; table: string; index: string; scans: number; fetches: number; size: number; valid: boolean; ready: boolean }
...
async function load() {
  ...
  const [idxScans, idxSize, idxFetches, idxValid, idxReady, seq, idxScansT, live, tblSize] = await Promise.all([
    fetchMap('postgresql_index_scans', idxKey),
    fetchMap('postgresql_index_size', idxKey),
    fetchMap('postgresql_index_tup_fetch', idxKey),
    fetchMap('postgresql_index_valid', idxKey),
    fetchMap('postgresql_index_ready', idxKey),
  ...
  indexes.value = Object.keys({ ...idxScans, ...idxSize, ...idxValid, ...idxReady }).map((k) => {
    const [schema = '', table = '', index = ''] = k.split('|')
    return { schema, table, index, scans: idxScans[k] ?? 0, fetches: idxFetches[k] ?? 0, size: idxSize[k] ?? 0, valid: (idxValid[k] ?? 1) > 0, ready: (idxReady[k] ?? 1) > 0 }
  })
...
<template>
  <div>
    ...
    >
      ...
      <template #cell-health="{ row }"><span :class="['health', { bad: !indexRow(row).valid || !indexRow(row).ready }]">{{ indexRow(row).valid ? (indexRow(row).ready ? 'usable' : 'building') : 'invalid' }}</span></template>
...
.health { color: var(--ok, #22c55e); font-size: 12px; }
.health.bad { color: var(--error, #ef4444); }
</style>
