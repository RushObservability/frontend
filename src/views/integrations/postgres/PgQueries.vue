  ...
  try {
    ...
    rows.value = Array.from(latest.values()).map((r) => {
      ...
      return {
        ...
        wal_bytes: num(a['wal_bytes']),
        parallel_workers: num(a['parallel_workers_launched']),
        sql: r.Body,
        ts: r.Timestamp,
      }
    })
      ...
      <template #row-detail="{ row }">
              <div class="q-panel">
                ...
                <div class="q-stats">
                  ...
                  <div class="q-stat">
                    <div class="q-lbl">Planning</div>
                    <div class="q-val small">{{ queryRow(row).plan_ms.toFixed(1) }} ms · {{ Math.round(queryRow(row).plans).toLocaleString() }} plans</div>
                  </div>
                  <div class="q-stat">
                    <div class="q-lbl">WAL generated</div>
                    <div class="q-val small">{{ Math.round(queryRow(row).wal_bytes).toLocaleString() }} B</div>
                  </div>
                  <div class="q-stat">
                    <div class="q-lbl">Parallel workers</div>
                    <div class="q-val small">{{ Math.round(queryRow(row).parallel_workers).toLocaleString() }}</div>
                  </div>
                </div>
