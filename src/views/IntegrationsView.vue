...
const primaryPages = computed(() => {
  ...
  return addon.value.pages.filter((page) => visible.has(page.key))
})
const morePages = computed(() => {
  if (!addon.value || !condensedNavigation.value) return []
  const primary = new Set(primaryPages.value.map((page) => page.key))
  return addon.value.pages.filter((page) => !primary.has(page.key))
})
const moreGroups = computed(() => {
  const groups = new Map<string, typeof morePages.value>()
  for (const page of morePages.value) {
...
const activeMorePage = computed(() => morePages.value.some((page) => page.key === activePage.value?.key))

function closeMoreViews(event: MouseEvent) {
    ...
    <section class="integrations-main">
      <template v-if="addon && available">
        ...
        <nav class="page-tabs" role="tablist" :aria-label="addon.label + ' views'">
          ...
          >{{ p.label }}</router-link>
          <details v-if="condensedNavigation" class="page-more" :class="{ active: activeMorePage }">
            <summary class="page-more-trigger">More views <span class="page-more-count">{{ morePages.length }}</span><span class="page-more-chevron" aria-hidden="true">⌄</span></summary>
            <div class="page-more-menu">
              <div v-for="group in moreGroups" :key="group.label" class="page-more-group">
                <div class="page-more-label">{{ group.label }}</div>
                <router-link
                  v-for="p in group.pages"
                  :key="p.key"
                  :to="{ name: 'integration-page', params: { addon: addon.key, page: p.key }, query: route.query }"
                  :class="['page-more-item', { active: p.key === activePage?.key }]"
                  @click="closeMoreViews"
                >{{ p.label }}</router-link>
              </div>
            </div>
          </details>
