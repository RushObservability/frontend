...
test('service browser gives the unfiltered page a useful starting point', async ({ page }, info) => {
  ...
  await page.screenshot({ path: info.outputPath('profiles-services.png'), fullPage: true })
  await page.getByRole('searchbox', { name: 'Find a service' }).fill('missing')
  await expect(page.getByText('No services match your search')).toBeVisible()
  await page.getByRole('searchbox', { name: 'Find a service' }).fill('check')
